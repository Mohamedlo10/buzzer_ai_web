...
function ModeratorGameScreen({ q, index, total, rows, setRows, you, onNext, last }) {
      ...
      <div className="s-body" style={{ gap:12 }}>
        ...
        <div className="card" style={{ overflow:'hidden', padding:0 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 14px', borderBottom:'1px solid var(--line)', background:'color-mix(in oklab,var(--energy) 5%,transparent)' }}>
            ...
            <button onClick={()=>setCorrectMode(v=>!v)} className="chip" style={{ cursor:'pointer', padding:'4px 10px', background:correctMode?'var(--accent)':'var(--surface-2)', color:correctMode?'#11112a':'var(--txt-60)' }}>{correctMode?'Terminé':'✎ Corriger'}</button>
          </div>
          {sorted.map((r,i)=>{
              ...
              <div key={r.name} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 14px', borderBottom:i<sorted.length-1?'1px solid var(--line)':'none', background:isYou?'color-mix(in oklab,var(--accent) 9%,transparent)':'transparent' }}>
                ...
                {correctMode ? (
                  <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                    <button onClick={()=>award(r.name,-100)} className="iconbtn" style={{ width:26, height:26, borderRadius:8, background:'color-mix(in oklab,var(--buzz) 20%,transparent)', color:'var(--buzz)', fontWeight:700 }}>−</button>
                    <span style={{ fontFamily:'var(--font-display)', fontWeight:600, fontSize:13, minWidth:38, textAlign:'center' }}>{r.score}</span>
                    <button onClick={()=>award(r.name,100)} className="iconbtn" style={{ width:26, height:26, borderRadius:8, background:'color-mix(in oklab,var(--accent) 20%,transparent)', color:'var(--accent)', fontWeight:700 }}>+</button>
                  </div>
                ) : (
