...
function GameScreen({ mode, q, index, total, rows, you, onBuzz, onTimeout, onAdvance }) {
  ...
  uE(() => {
    if (buzzed || mState!=='idle') return;
    const t = setTimeout(()=>{
      const rival = { name:'Sofia', ms:820+Math.floor(Math.random()*400) };
      setQueue([rival]);
  ...
  const buzz = () => {
    if (buzzed || mState!=='idle') return;
    setBuzzed(true);
    setQueue([{ name:'Toi', ms:300+Math.floor(Math.random()*240), you:true }]);
  ...
  const buzzerDisabled = buzzed || mState==='wrong' || mState==='rival';
      ...
      <div className="s-body" style={{ gap:14 }}>
        ...
        {mState==='idle' || mState==='rival' ? (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:14, padding:'6px 0' }}>
            <Buzzer disabled={buzzerDisabled} onClick={buzz}/>
        ...
        {/* buzz queue */}
        <BuzzQueue queue={queue}/>

        {/* live leaderboard */}
        <LiveLeaderboard rows={rows} you={you}/>
      </div>
    ...
    <div style={{ borderRadius:18, overflow:'hidden', border:`1px solid ${queue.length?'var(--accent)':'var(--line)'}`, background: queue.length?'color-mix(in oklab,var(--accent) 6%,transparent)':'var(--surface)' }}>
      ...
      {queue.length>0 ? queue.map((b,i)=>(
        <div key={i} style={{ display:'flex', alignItems:'center', gap:11, padding:'11px 13px', background:'color-mix(in oklab,var(--accent) 8%,transparent)' }}>
          <div style={{ width:40, height:40, borderRadius:'50%', display:'grid', placeItems:'center', background:'var(--accent)', color:'#11112a', fontFamily:'var(--font-display)', fontWeight:700, fontSize:17 }}>1</div>
          <div style={{ flex:1 }}>
            <div style={{ fontWeight:700, fontSize:15, color:b.you?'var(--energy)':'var(--txt)' }}>{b.name}{b.you?' (Vous)':''}</div>
            <div style={{ fontSize:12.5, color:'var(--accent)' }}>En train de répondre</div>
          </div>
...
/* ───────────────── YOUR TURN (host / sans modérateur : choix A/B/C/D) ───────────────── */
function YourTurnScreen({ q, onAnswer }) {
  ...
  const pick = (i)=>{ if(picked!=null)return; setPicked(i); setTimeout(()=>onAnswer(i,sec),360); };
  const LBL=['A','B','C','D'];
  return (
    <div className="screen">
      <div className="s-head" style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <span className="chip acc" style={{ background:'var(--accent)', color:'#11112a' }}><span className="dotpulse" style={{ background:'#11112a' }}/> C'est ton tour</span>
        <Ring seconds={sec} total={TOTAL} size={46}/>
      </div>
      <div className="s-body" style={{ gap:16, justifyContent:'center' }}>
        <div style={{ animation:'rise .4s both' }}>
          <div className="eyebrow" style={{ marginBottom:8 }}>{q.cat}</div>
          <h2 style={{ fontSize:21, lineHeight:1.25, textWrap:'pretty' }}>{q.q}</h2>
        </div>
        <div style={{ marginTop:2 }}><TimerBar seconds={sec} total={TOTAL}/></div>
          ...
          {q.opts.map((o,i)=>{
            ...
            return (
              <button key={i} onClick={()=>pick(i)} disabled={picked!=null} style={{
                appearance:'none', cursor:picked!=null?'default':'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10, padding:'14px 12px', borderRadius:14, minHeight:58,
                background: on?'color-mix(in oklab,var(--accent) 18%,var(--surface))':'var(--surface)', border:`1.5px solid ${on?'var(--accent)':'var(--line)'}`, opacity:dim?.45:1, transition:'all .18s', color:'var(--txt)', fontFamily:'var(--font-ui)' }}>
                <span style={{ width:30, height:30, borderRadius:9, flexShrink:0, display:'grid', placeItems:'center', background:'var(--surface-2)', fontWeight:700, fontSize:13 }}>{LBL[i]}</span>
                <span style={{ fontSize:14.5, fontWeight:600, lineHeight:1.2 }}>{o}</span>
              </button>
            );
          })}
        </div>
        <p className="muted" style={{ textAlign:'center', fontSize:12 }}>Réponds vite pour maximiser tes points</p>
      </div>
    </div>
  );
}
