...
function App() {
  ...
  const replay = ()=>{ setQi(0); setRows(newRows(you)); setStats({correct:0,total:0,base:0,corr:0,debt:-450}); setPhase('lobby'); };
  const goHome = ()=>{ setPhase('dashboard'); setTab('home'); };
  const startCreate = ()=>{ setRows(newRows(you)); setQi(0); setStats({correct:0,total:0,base:0,corr:0,debt:-450}); setPhase('categories'); };
  const doJoin = (c)=>{ setShowJoin(false); setRows(newRows(you)); setQi(0); setPhase('categories'); };
  ...
  if (phase==='dashboard') {
  ...
  else if (phase==='categories') screen=<CategoriesScreen initial={myCats} onJoin={(sel)=>{ if(sel&&sel.length)setMyCats(sel); setPhase('lobby'); }}/>;
    ...
    if (t.role==='mod') screen=<ModeratorGameScreen q={q} index={qi} total={QUESTIONS.length} rows={rows} setRows={setRows} you={you} last={qi>=QUESTIONS.length-1} onNext={nextQuestion}/>;
    else screen=<GameScreen mode={t.mode} q={q} index={qi} total={QUESTIONS.length} rows={rows} you={you} onBuzz={()=>setPhase('yourturn')} onTimeout={()=>finishRound(null,0,true)} onAdvance={advanceMod}/>;
  }
  ...
  else if (phase==='results') screen=<ResultsScreen rows={rows} you={you} stats={{...stats, debt:-450}} code={code} onReplay={replay}/>;
  ...
  const STEPS = ['dashboard','categories','lobby','generating','game','yourturn','reveal','results'];
  const SEED = [2100,2750,1850,1500,1200,900,600,300]; // Toi=2100 → 2nd place
  const seedRows = ()=> newRows(you).map((x,i)=>({...x, score:SEED[i]||100, lastGain:[0,300,150,0,200,0,100,0][i]||0, delta:[1,0,-1,2,0,0,0,0][i]||0}));
  const jump = (p)=>{
    if(p==='reveal') setQi(0);
    if(p==='game' || p==='results'){
      setQi(p==='game'?0:qi);
