    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <button
          ...
          className="inline-flex items-center gap-2 text-txt-60 hover:text-txt transition-colors text-sm w-fit cursor-pointer"
        >
          <ArrowLeft size={16} />
          Retour à la liste
        </button>
      ...
      <Card>
        <div className="flex flex-col sm:flex-row items-start gap-5">
          <div className="relative shrink-0">
            <Avatar url={user.avatarUrl} username={user.username} size={72} />
            <span
          ...
          <div className="flex flex-wrap gap-2 shrink-0">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-xl bg-surface-2 hover:bg-surface-2 text-txt text-sm transition-colors cursor-pointer"
              >
                <ShieldCheck size={16} />
                Changer rôle
                <ChevronDown size={14} />
                    ...
                    {ALL_ROLES.map((r) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-2 px-3 py-2.5 text-left text-sm transition-colors cursor-pointer ${
                            user.role === r
            ...
            {user.banned ? (
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-xl bg-accent/15 hover:bg-accent/20 text-accent text-sm transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-xl bg-danger/15 hover:bg-danger/20 text-danger text-sm transition-colors cursor-pointer"
              >
                <ShieldX size={16} />
                Bannir
              </button>
                ...
                <tbody>
                  {user.recentSessions.map((s) => (
                    <tr key={s.sessionId} className="border-b border-line last:border-b-0">
                      ...
                      <td className="px-3 py-2.5 text-sm text-txt">
                        {s.rank ? `${s.rank}/${s.totalPlayers}` : '—'}
                      </td>
            ...
            {user.friends.length === 0 ? (
              ...
              <div className="space-y-3">
                {user.friends.map((f) => (
                  <div
                    key={f.userId}
                    className="flex items-center gap-3 p-2 rounded-xl hover:bg-surface-2 transition-colors cursor-pointer"
                    onClick={() => navigate(`/users/${f.userId}`)}
                  >
                    <Avatar url={f.avatarUrl} username={f.username} size={36} />
