...
> = {
  LOBBY: { label: 'Lobby', color: 'var(--primary)', bg: 'rgba(224, 86, 36, 0.125)', icon: Users },
  GENERATING: { label: 'Génération', color: 'var(--gold)', bg: 'rgba(217, 119, 6, 0.125)', icon: Zap },
  PLAYING: { label: 'En cours', color: 'var(--indigo)', bg: 'rgba(78, 140, 255, 0.125)', icon: Swords },
  PAUSED: { label: 'Pause', color: 'var(--warn)', bg: 'rgba(243, 156, 18, 0.125)', icon: Clock },
  RESULTS: { label: 'Terminée', color: 'var(--silver)', bg: 'rgba(192, 192, 192, 0.125)', icon: Trophy },
  CANCELLED: { label: 'Annulée', color: 'var(--bad)', bg: 'rgba(231, 76, 60, 0.125)', icon: XCircle },
};
...
export function SessionDetailPage() {
  ...
  const { data: session, isLoading } = useQuery({
    ...
    queryFn: () => adminApi.getAdminSessionDetail(sessionId!),
    enabled: !!sessionId,
  });
  ...
  const questionColumns: Column<AdminSessionQuestion>[] = [
    ...
    },
    {
      key: 'text',
      header: 'Question',
      render: (row) => <span className="text-txt text-sm line-clamp-2">{row.text}</span>,
    },
    {
      key: 'answer',
      header: 'Réponse',
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-txt text-2xl font-bold tracking-wider font-display">
                #{session.code}
              </h1>
              <span
              ...
              {session.isPrivate && (
                <span className="text-xs px-2 py-0.5 rounded-full bg-surface-2 text-txt-60">
                  Privée
                </span>
              )}
            </div>
        ...
        {canStop && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-buzz/15 rounded-xl hover:bg-buzz/20 transition-colors disabled:opacity-40 cursor-pointer"
          >
