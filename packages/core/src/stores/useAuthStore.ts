...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  isLoading: true, // true until restoreSession() completes on app start
  ...
  restoreSession: async () => {
    try {
      ...
      if (!token) {
        ...
        // No token = user has no valid session (never logged in or fully logged out).
        // Reset onboarding so they go through the welcome flow → login/register.
        await appStorage.clearOnboardingDone();
        return false;
      }
      ...
      // Always unblock the UI — isLoading must return to false regardless of outcome
      set({ isLoading: false });
    }
  },
